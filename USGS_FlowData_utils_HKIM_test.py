...
Q = site.getDailyDischarge().rename(columns={'Flow ({})'.format(site.getUnit()):'Flow'})
target_alpha = 0.05
time_scale = 'Y'
unit = site.getUnit()
def trendTest(Q, time_scale, target_alpha):

    #startdate = pd.to_datetime(start_date)
    #enddate = pd.to_datetime(end_date)
    
    t_Q = Q.rename(columns={'Flow ({})'.format(unit):'Flow'})
    
    # If we have a date gap, we should add a code for it
    # Here, I assume that USGS provides continuous data
    valid_flag = True
    if time_scale == 'M':
        t_Q_aggr = t_Q.groupby(t_Q.Date.dt.strftime('%Y-%m')).Flow.agg(['mean'])
        ...
        if nod < 120: # should have more than 10-year lenth of data
            valid_flag = False
    
    elif time_scale == 'Y':        
        t_Q_aggr = t_Q.groupby(t_Q.Date.dt.strftime('%Y')).Flow.agg(['mean'])
        nod = len(t_Q_aggr)
        if nod < 0: # should have more than 10-year lenth of data
            valid_flag = False
    ...
    if valid_flag:
        ...
        if (R_MK[3] < target_alpha) & (R_MK[1] > 0):
            ...
            trend_result = 0 # no trend
            slope_result = 0
    else: # if we don't have enough data, we cannot conduct the trend analysis
        trend_result = False
        slope_result = False

    return trend_result, slope_result, R_TS, R_MK

results = trendTest(Q, time_scale, target_alpha)
print(results)
